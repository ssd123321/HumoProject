...
func (c *Cache) GetPerson(ctx context.Context) (*model.Person, error) {
	...
	err := c.client.Get(ctx, fmt.Sprint(ctx.Value("id").(int))).Scan(&personJSON)
	if err != nil {
...
func (c *Cache) SetPerson(ctx context.Context, person *model.Person) error {
	...
	err = c.client.Set(ctx, fmt.Sprint(ctx.Value("id").(int)), string(payload), time.Second*20).Err()
	if err != nil {
...
func (c *Cache) DeletePerson(ctx context.Context) error {
	err := c.client.Del(ctx, fmt.Sprint(ctx.Value("id").(int))).Err()
	if err != nil {
