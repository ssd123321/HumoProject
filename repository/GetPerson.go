...
func (r *Repository) GetPersonByID(ctx context.Context) (*model.Person, error) {
	...
	err := r.DB.Raw("Select id, content::text, deleted_at, created_at, updated_at, cache From person where id = ? and deleted_at is null", ctx.Value("id")).First(&person).Error
	if err != nil {
